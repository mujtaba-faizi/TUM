...
object ReturnTrips {
  ...
  def compute(trips : Dataset[Row], dist : Double, spark : SparkSession) : Dataset[Row] = {
    ...
    val tripsSubsetExtended = tripsSubset.withColumn("pickup_time_bucket",floor($"pickup_time"/ hours))
      .withColumn("dropoff_time_bucket",floor($"dropoff_time" / hours))
    //.withColumn("pickup_latitude_b",floor($"pickup_latitude_a"/ (lit(dist) / (R * 1000))))
    ...
    val tripsJoin = tripsBucket.as("a").join(tripsSubsetExtended.as("b"),
      //($"a.pickup_latitude_b" === $"b.dropoff_latitude_b")
      //&& ($"a.dropoff_latitude_b"=== $"b.pickup_latitude_b") &&
      ($"a.dropoff_time_bucket" === $"b.pickup_time_bucket"))

    // applying constraints
    // filtering after adding all columns somehow makes it faster?

    val result = tripsJoin
      .filter(distance($"a.dropoff_latitude_a", $"a.dropoff_longitude_a", $"b.pickup_latitude_a", $"b.pickup_longitude_a") < lit(dist) &&
        distance($"b.dropoff_latitude_a", $"b.dropoff_longitude_a", $"a.pickup_latitude_a", $"a.pickup_longitude_a") < lit(dist))
      .filter(($"a.dropoff_time" + hours) > $"b.pickup_time")
