...
object ReturnTrips {
  ...
  def compute(trips : Dataset[Row], dist : Double, spark : SparkSession) : Dataset[Row] = {
    ...
    val distBuck = tripsInSec
      .withColumn("pickup_time_bucket1", floor($"pickup_datetime" / hours))
      .withColumn("dropoff_time_bucket1", floor($"dropoff_datetime" / hours))
    ...
    val distBuckNeighbors = broadcast(distBuck
      .withColumn("pickup_time_bucket", explode(array($"pickup_time_bucket1"-1, $"pickup_time_bucket1", $"pickup_time_bucket1"+1)))
      //.withColumn("dropoff_time_bucket1", explode(array($"dropoff_time_bucket1")))
    ...
    //.cache()
    distBuckNeighbors.show(10)
    //distBuckNeighbors.show(1000)

    val result = distBuck.as("a").join(distBuckNeighbors.as("b"),
      //&&
      //($"a.pickup_lat_bucket" === $"b.dropoff_lat_bucket")
      //&& ($"b.pickup_lat_bucket" === $"a.dropoff_lat_bucket")
      ($"a.dropoff_time_bucket1" === $"b.pickup_time_bucket1")
      ...
      .filter(($"a.dropoff_datetime" < $"b.pickup_datetime")
        ...
        // && ($"b.pickup_datetime" - $"a.dropoff_datetime" < hours)
        && (makeDistExpr($"a.pickup_latitude", $"a.pickup_longitude", $"b.dropoff_latitude", $"b.dropoff_longitude") < r)
        && (makeDistExpr($"b.pickup_latitude", $"b.pickup_longitude", $"a.dropoff_latitude", $"a.dropoff_longitude") < r)
      )
